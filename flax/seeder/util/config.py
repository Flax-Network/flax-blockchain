...
def patch_default_seeder_config(root_path: Path, filename="config.yaml") -> None:
    ...
    if "seeder" in existing_config:
        print("Flax Seeder section exists in config. No action required.")
        return

    print("Flax Seeder section does not exist in config. Patching...")
    config = load_config(root_path, "config.yaml")
    # The following ignores root_path when the second param is absolute, which this will be
    seeder_config = load_config(root_path, pkg_resources.resource_filename("flax.util", "initial-config.yaml"))
