...
from flax.protocols.full_node_protocol import RequestPeers, RespondPeers
...
class FullNodeDiscovery:
    ...
    async def update_peer_timestamp_on_message(self, peer: WSFlaxConnection) -> None:
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self) -> None:
        ...
        async def on_connect(peer: WSFlaxConnection) -> None:
    ...
    async def on_connect_callback(self, peer: WSFlaxConnection) -> None:
