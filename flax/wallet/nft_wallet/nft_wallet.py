...
from flax.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from flax.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
...
class NFTWallet:
    ...
    async def mint_from_did(
        ...
        xfx_coins: Optional[Set[Coin]] = None,
        xfx_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an xfx spend that funds the transactions and pays fees.
        ...
        :param xfx_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param xfx_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the xfx spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an xfx coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if xfx_coins is None:
            xfx_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(xfx_coins) > 0
        ...
        # Create the xfx spend to fund the minting.
        spend_value = sum([coin.amount for coin in xfx_coins])
        change: uint64 = uint64(spend_value - total_amount)
        xfx_spends = []
        if xfx_change_ph is None:
            xfx_change_ph = await self.standard_wallet.get_new_puzzlehash()
        xfx_primaries = [
            AmountWithPuzzlehash({"puzzlehash": xfx_change_ph, "amount": change, "memos": [xfx_change_ph]})
        ]

        first = True
        for xfx_coin in xfx_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(xfx_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in xfx_coins]
                message_list.append(
                    Coin(xfx_coin.name(), xfx_primaries[0]["puzzlehash"], xfx_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(xfx_coins) > 1:
                    xfx_announcement: Optional[Set[bytes]] = {message}
                else:
                    xfx_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=xfx_primaries,
                    fee=fee,
                    coin_announcements=xfx_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(xfx_coin.name(), message).name()
            ...
            xfx_spends.append(CoinSpend(xfx_coin, puzzle, solution))
        xfx_spend = await self.standard_wallet.sign_transaction(xfx_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, xfx_spend, *eve_spends])
        return total_spend

    async def mint_from_xfx(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single XFX spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = xfx_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from XFX to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the XFX spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the XFX spend
            ...
            # Annnouncements for eve spend. These are asserted by the xfx spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for xfx_coin in xfx_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=xfx_primaries + primaries,
                    fee=fee,
                    coin_announcements=xfx_announcement if len(xfx_coins) > 1 else None,
