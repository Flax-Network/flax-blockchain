...
from flax.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
...
class WalletNode:
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSFlaxConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def fetch_puzzle_solution(self, peer: WSFlaxConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSFlaxConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSFlaxConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
