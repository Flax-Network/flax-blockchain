...
from flax.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from flax.cmds.configure import configure
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xfx_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xfx_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xfx_target_address") or all_targets[-2] == config["pool"].get(
                    "xfx_target_address"
                ):
        ...
        if "xfx_target_address" not in config["farmer"]:
            print(
                f"Setting the xfx destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if "xfx_target_address" not in config["pool"]:
            print(f"Setting the xfx destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        ...
        if updated_target:
            print(
                f"To change the XFX destination addresses, edit the `xfx_target_address` entries in"
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(flax_ca_crt_path, flax_ca_crt, flax_ca_key_path, flax_ca_key, overwrite=overwrite)
...
def init(
    ...
    if create_certs is not None:
        ...
        return flax_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def flax_init(
    ...
    *,
    ...
    """
    flax_root = os.environ.get("FLAX_ROOT", None)
    if flax_root is not None:
        print(f"FLAX_ROOT is set to {flax_root}")
