...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true electron-builder build --linux deb --arm64 \
    --config.extraMetadata.name=flax-blockchain \
    --config.productName="$PRODUCT_NAME" --config.linux.desktop.Name="Flax Blockchain" \
    --config.deb.packageName="flax-blockchain"
...
GUI_DEB_NAME=flax-blockchain_${FLAX_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${FLAX_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
