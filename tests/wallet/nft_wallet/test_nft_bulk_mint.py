...
async def test_nft_mint_from_did(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.flaxnetwork.org/img/branding/flax-logo.svg"]), ("h", token_bytes(32).hex())]
            ),
...
@pytest.mark.asyncio
async def test_nft_mint_from_did_multiple_xfx(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
...
@pytest.mark.asyncio
async def test_nft_mint_from_xfx(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
...
@pytest.mark.asyncio
async def test_nft_mint_from_xfx_multiple_xfx(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
