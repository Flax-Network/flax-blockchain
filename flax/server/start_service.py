...
from flax.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from flax.server.flax_policy import set_flax_policy
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSFlaxConnection], Awaitable[None]]] = None,
        ...
        self._server = FlaxServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_flax_policy(connection_limit)
