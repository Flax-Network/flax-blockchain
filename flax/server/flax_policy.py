...
class FlaxSelectorEventLoop(asyncio.SelectorEventLoop):
...
if sys.platform == "win32":
    ...
    class FlaxProactor(asyncio.windows_events.IocpProactor):
        ...
        async def _flax_accept_loop(self, listener):
            while True:
        ...
        def accept(self, listener):
            coro = self._flax_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class FlaxProactorEventLoop(asyncio.windows_events.ProactorEventLoop):
...
class FlaxPolicy(asyncio.DefaultEventLoopPolicy):
...
def set_flax_policy(connection_limit: int) -> None:
