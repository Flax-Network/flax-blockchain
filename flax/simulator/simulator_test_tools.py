...
from flax.simulator.ssl_certs import get_next_nodes_certs_and_keys, get_next_private_ca_cert_and_key
...
def create_config(flax_root: Path, fingerprint: int) -> Dict[str, Any]:
...
async def get_full_flax_simulator(
    ...
    if config is None:
        config = create_config(flax_root, fingerprint)
