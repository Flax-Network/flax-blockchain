...
from flax.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    create_fullpuz,
    match_did_puzzle,
    program_to_metadata,
...
from flax.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: Optional[WSFlaxConnection], coin_id: bytes32, latest: bool = True
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        # signatures made by `flax keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XFX.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xfx_coin_list = request.get("xfx_coins", None)
        xfx_coins = None
        if xfx_coin_list:
            xfx_coins = set([Coin.from_json_dict(xfx_coin) for xfx_coin in xfx_coin_list])
        xfx_change_target = request.get("xfx_change_target", None)
        if xfx_change_target is not None:
            if xfx_change_target[:2] == "xfx":
                xfx_change_ph = decode_puzzle_hash(xfx_change_target)
            else:
                xfx_change_ph = bytes32(hexstr_to_bytes(xfx_change_target))
        else:
            xfx_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_xfx(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
