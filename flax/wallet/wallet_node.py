...
from flax.protocols.wallet_protocol import CoinState, RespondBlockHeader, RespondToCoinUpdates
...
from flax.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from flax.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_full_node_peers_in_order(self) -> List[WSFlaxConnection]:
        ...
        all_nodes: List[WSFlaxConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSFlaxConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSFlaxConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSFlaxConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
