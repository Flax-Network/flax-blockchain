...
import flax.cmds.configure as flax_configure
from flax import __version__
from flax.cmds.flax import monkey_patch_click
from flax.cmds.init_funcs import init
from flax.seeder.util.config import patch_default_seeder_config
from flax.seeder.util.service_groups import all_groups, services_for_groups
from flax.seeder.util.service import launch_service, kill_service
...
@click.group(
    help=f"\n  Manage the Flax Seeder ({__version__})\n",
    epilog="Try 'flax seeder start crawler' or 'flax seeder start server'",
...
@cli.command("version", short_help="Show the Flax Seeder version")
...
def init_cmd(ctx: click.Context, **kwargs):
    print("Calling Flax Seeder Init...")
    init(None, ctx.obj["root_path"], True)
    if os.environ.get("FLAX_ROOT", None) is not None:
        print(f"warning, your FLAX_ROOT is set to {os.environ['FLAX_ROOT']}.")
    root_path = ctx.obj["root_path"]
    print(f"Flax directory {root_path}")
    if root_path.is_dir() and not Path(root_path / "config" / "config.yaml").exists():
        # This is reached if FLAX_ROOT is set, but there is no config
        # This really shouldn't happen, but if we dont have the base flax config, we can't continue
...
def configure(
    ...
    flax_configure.configure(root_path, "", "", "", "", "", "", "", "", testnet, "")

    config: Dict = load_config(DEFAULT_ROOT_PATH, "config.yaml")
    change_made = False
    if testnet is not None:
        if testnet == "true" or testnet == "t":
            print("Updating Flax Seeder to testnet settings")
            port = 58444
            network = "testnet10"
            bootstrap = ["testnet-node.flaxnetwork.org"]
            ...
            print("Updating Flax Seeder to mainnet settings")
            port = 8444
            network = "mainnet"
            bootstrap = ["node.flaxnetwork.org"]
    ...
    if change_made:
        print("Restart any running Flax Seeder services for changes to take effect")
