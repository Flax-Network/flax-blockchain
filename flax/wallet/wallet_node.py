...
from flax.full_node.weight_proof import chunks
...
from flax.types.weight_proof import WeightProof, SubEpochData
from flax.util.byte_types import hexstr_to_bytes
from flax.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from flax.util.default_root import STANDALONE_ROOT_PATH
from flax.util.ints import uint32, uint64
from flax.util.keychain import KeyringIsLocked, Keychain
...
class WalletNode:
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSFlaxConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    async def trusted_sync(self, full_node: WSFlaxConnection):
        """
    ...
    async def receive_state_from_peer(
        self, items: List[CoinState], peer: WSFlaxConnection, fork_height: Optional[uint32], height: Optional[uint32]
    ):
        assert self.wallet_state_manager is not None
        trusted = self.is_trusted(peer)
    ...
    async def subscribe_to_phs(self, puzzle_hashes: List[bytes32], peer: WSFlaxConnection, height=uint32(0)):
        """
    ...
    async def subscribe_to_coin_updates(self, coin_names: List[bytes32], peer: WSFlaxConnection, height=uint32(0)):
        """
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: Optional[WSFlaxConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSFlaxConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSFlaxConnection) -> Optional[HeaderBlock]:
    ...
    async def untrusted_sync_to_peer(self, peer: WSFlaxConnection, syncing: bool, fork_height: int):
        assert self.wallet_state_manager is not None
