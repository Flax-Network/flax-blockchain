...
from flax.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from flax.server.ws_connection import ConnectionCallback, WSFlaxConnection
from flax.types.blockchain_format.sized_bytes import bytes32
from flax.types.peer_info import PeerInfo
from flax.util.errors import Err, ProtocolError
from flax.util.ints import uint16
from flax.util.network import WebServer, is_in_network, is_localhost
...
class FlaxServer:
    ...
    all_connections: Dict[bytes32, WSFlaxConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        flax_ca_crt_path, flax_ca_key_path = flax_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                flax_ca_crt_path, flax_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                flax_ca_crt_path, flax_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Optional[ConnectionCallback] = None) -> None:
        if self.webserver is not None:
            raise RuntimeError("FlaxServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSFlaxConnection.create(
                self._local_type,
                ws,
                self.api,
                self._port,
    ...
    async def connection_added(
        self, connection: WSFlaxConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSFlaxConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSFlaxConnection, trusted_peers: Dict[str, Any]) -> bool:
