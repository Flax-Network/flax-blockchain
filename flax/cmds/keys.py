...
def keys_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with flax init")
...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older Flax versions use "
...
@keys_cmd.command("migrate", short_help="Attempt to migrate keys to the Flax keyring")
@click.pass_context
...
@click.option("--prefix", "-x", help="Address prefix (xfx for mainnet, txfx for testnet)", default=None, type=str)
@click.pass_context
def search_cmd(
    ctx: click.Context,
